...
class CategoryListController extends Controller
{
    public function show(Category $category, Request $request)
    {
        ...
        $category->load(['media', 'subcategories']);

        // ✅ Banner ONLY (category_banner)
        $banner = optional($category->getFirstMedia('category_banner'))->getUrl();

        // ✅ per page: default 16 (desktop), UI will send 8 for mobile
        ...
        $q = Product::query()
            ->where('status', 1)
            ->whereHas('categories', function ($qq) use ($category) {
                $qq->where('categories.id', $category->id);
            })
            ->with(['media']); // keep simple

        // Subcategory filter (supports common schemas)
        if ($subId) {
        ...
        // Brand filter (supports common schemas)
        if (!empty($brandIds)) {
        ...
        // Products payload for CategoryNav-style cards
        ...
        // Sidebar: brands (WITHOUT Brand::products())
        $brands = collect([]);
        if (Schema::hasColumn('products', 'brand_id')) {
            $brandIdsInCat = Product::where('status', 1)
            ...
            if ($brandIdsInCat->count()) {
                $brands = Brand::where('status', 1)
                    ->whereIn('id', $brandIdsInCat)
                    ->orderBy('title')
                    ->get()
                    ->map(fn($b) => [
                        'id' => $b->id,
