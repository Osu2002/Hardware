...
class CustomerController extends Controller
{
    ...
    public function getData()
    {
        ...
        if ($user->roles[0]->name == 'Super Admin') {
            $customers = Customer::with('media');
        } elseif ($user->roles[0]->name == 'Dealer') {
            $customers = Customer::where(['dealer_id' => $user->id])->with('media');
        }
    ...
    public function update(Request $request)
    {
        ...
        $request->validate([
            'name' => ['required'],
            'status' => ['required'],
            'email' => ['required', 'email', Rule::unique('customers', 'email')->ignore($request->id)], // Exclude current user
            'phone' => ['required'],
            'address' => ['required'],
