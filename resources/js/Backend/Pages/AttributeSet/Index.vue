        ...
        <div class="card mb-4">
          <div class="card-header pb-0 d-flex align-items-center justify-content-between">
            <div>
              ...
              <p>Group attributes for each product family.</p>
            </div>
            <Link class="btn btn-main btn-sm" :href="route('attribute-set.create')">
          ...
          <div class="card-body">
            <data-table
              ref="datatable"
              :id="'mytable'"
              :url="route('attribute-set.getdata')"
              :columns="columns"
        ...
        <div class="modal-dialog modal-sm" role="document">
          <div class="modal-content">
            ...
            <form @submit.prevent="deleteSelectedItems">
              <div class="modal-body py-0 px-3"><p>Delete selected set(s)?</p></div>
...
export default {
  ...
  mounted() {
    $("#mytable tbody").on("click", "tr .action_delete", (evt) => {
      ...
      this.selectedRows = []; this.getSelectedItems(id);
    });
    $("#mytable tbody").on("click", "tr .action_edit", (evt) => {
      const id = $(evt.target).data("item-id");
      this.$inertia.visit(route("attribute-set.edit", id));
    });
    ...
    $("#mytable tbody").on("click", ".item-check input[type=checkbox]", (evt) => {
      ...
      if ($(evt.target).prop("checked") && !this.selectedRows.includes(v)) this.getSelectedItems(v);
      else this.removeUnselectedItem(v);
    });
  },
  methods: {
    ...
    updateStatus(id, status) {
      ...
      this.form.put(route("attribute-set.change.status"), {
        preserveScroll: true,
    ...
    deleteSelectedItems() {
      this.form.transform(d => ({ ...d, ids: [...this.selectedRows] }))
        .post(route("attribute-set.delete"), {
