      ...
      <div class="col-md-12">
        <div class="card mb-4">
          <div class="card-header pb-0">
            <h5>Subcategory</h5>
            <p>Manage Subcategories by Category.</p>

            <div class="d-flex">
              <Link
                class="btn btn-main btn-sm"
                :href="route('subcategory.create')"
                v-if="$root.hasPermission('subcategory.create')"
              >
...
export default {
  ...
  data() {
    return {
      columns: [
        ...
        // badge html - don’t sort/search on it
        { mData: "status", name: "status", orderable: false, searchable: false },

        // IMPORTANT: must match DB column name
        { mData: "sort_order", name: "sort_order", orderable: true, searchable: false },

        { mData: "action", name: "action", orderable: false, searchable: false },
      ],
      columnDefs: [{ className: "text-center", targets: [0, 2, 3, 4] }],
      selectedRows: [],
      isDisabled: true,
  ...
  mounted() {
    $("#subcatTable tbody").on("click", "tr .action_delete", (evt) => {
      const id = $(evt.target).data("item-id");
      this.selectedRows = [];
      this.getSelectedItems(id);
    });

    $("#subcatTable tbody").on("click", "tr .action_edit", (evt) => {
      const id = $(evt.target).data("item-id");
      this.$inertia.visit(route("subcategory.edit", id));
    });

    $("#subcatTable tbody").on("click", "tr .action_status_change", (evt) => {
      const id = $(evt.target).data("item-id");
      const status = $(evt.target).data("status");
      this.updateStatus(id, status);
    });

    $("#subcatTable tbody").on("click", ".item-check input[type=checkbox]", (evt) => {
  ...
  methods: {
    ...
    getSelectedItems(value) {
      if (!this.selectedRows.includes(value)) this.selectedRows.push(value);
    ...
    updateStatus(id, status) {
      ...
      this.form.put(route("subcategory.change.status"), {
        preserveScroll: true,
        onSuccess: () => {
          this.form.reset();
          this.reloadTable();
