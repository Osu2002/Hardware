      ...
      <div class="col-md-12">
        <div class="card mb-4">
          <div class="card-header pb-0">
            ...
            <p>Manage Brands.</p>
            <div class="d-flex">
              <Link class="btn btn-main btn-sm" :href="route('brand.create')" v-if="$root.hasPermission('vehicle_type.view')">
          ...
          <div class="modal-content">
            ...
            <form id="formAccountDelete" @submit.prevent="deleteSelectedItems">
              <div class="modal-body py-0 px-3">
                <div class="row"><div class="col mb-1"><p>Are you sure you want to delete?</p></div></div>
              </div>
...
export default {
  ...
  data() {
    return {
      columns: [
        ...
        { mData: "image", name: "image", orderable: false, searchable: false },
  ...
  mounted() {
    ...
    $("#mytable tbody").on("click", "tr .action_edit", (evt) => {
      const id = $(evt.target).data("item-id");
      this.$inertia.visit(route("brand.edit", id));
    });
  ...
  methods: {
    ...
    getSelectedItems(v) { this.selectedRows.push(v); this.isDisabled = this.selectedRows.length === 0; },
    selectAll(evt) {
      const self = this;
      if ($(evt.target).is(":checked")) {
        $(".item-check input[type=checkbox]").prop("checked", true);
        $(".item-check input[type=checkbox]:checked").each(function () { if (!self.selectedRows.includes(this.value)) self.getSelectedItems(this.value); });
        this.isDisabled = false;
      } else {
        $(".item-check input[type=checkbox]").prop("checked", false);
        $(".item-check input[type=checkbox]").each(function () { if (self.selectedRows.includes(this.value)) self.removeUnselectedItem(this.value); });
        this.isDisabled = true;
      }
    },
    removeUnselectedItem(v) {
      this.selectedRows = this.selectedRows.filter(x => x != v);
      if (this.selectedRows.length <= 0) { $("#selectAll").prop("checked", false); this.isDisabled = true; }
    },
    updateStatus(id, status) {
      this.form.id = id; this.form.status = status;
      this.form.put(route("brand.change.status"), {
        preserveScroll: true,
