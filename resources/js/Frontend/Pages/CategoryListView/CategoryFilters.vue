...
export default {
  ...
  data() {
    return {
      ...
      // ✅ inner accordions (same as ProductFilter)
      subOpen: false,
      brandOpen: false,
      sortOpen: false,
  ...
  mounted() {
    if (typeof window !== "undefined" && window.matchMedia) {
      // ✅ sticky ONLY for large screens (<=1024 = tablet/mobile)
      ...
      const applyMode = (matches, force = false) => {
        ...
        // ✅ same behaviour as ProductFilter:
      ...
      // ✅ ensure per_page correct on first load (same as ProductFilter)
  ...
  methods: {
    ...
    onSortPick() {
      // close only sort accordion (same UX)
      this.sortOpen = false;
      this.$nextTick(() => this.syncShellHeight());
    },
...
.filters {
  ...
  /* ✅ same font stack you used everywhere */
