<template>
  <section class="category-products">
    <p v-if="!items.length" class="empty">No products found in this category.</p>
...
export default {
  ...
  methods: {
    // ✅ if user navigated from mobile to desktop and URL has per_page=8,
    // this will automatically correct to 16 (and reverse).
    ensurePerPage() {
      ...
      // If not set OR wrong, reload with correct per_page but preserve other filters
      if (!current || current !== desired) {
        url.searchParams.set("per_page", String(desired));

        // keep page number safe (reset to 1 when changing per_page)
    ...
    goToPage(link) {
      ...
      // ✅ keep correct per_page on pagination links too
      const desired = this.isMobile ? 8 : 16;
...
/* ✅ Grid rules: 4 desktop, 3 tablet, 2 mobile */
.grid {
  display: grid;
...
/* =========================
   ✅ CARD DESIGN: MATCH CategoryNav EXACTLY
   ========================= */
...
/* Typography exactly like CategoryNav */
.card-title {
  font-size: 0.85rem;
  font-weight: 500;
  color: #111827;
...
/* ✅ OLD PRICE (CategoryNav exact: custom strike line) */
.card-old-price {
  position: relative;
...
/* ✅ Mobile sizing exactly like CategoryNav mobile tighten */
@media (max-width: 768px) {
  .card-image-wrapper {
    padding-top: 62%;
  }
