    ...
    <div class="filters" :class="{ 'is-desktop-sticky': !isMobile }">
      ...
      <div class="filters-header">
        ...
        <div class="filters-actions">
          <!-- ✅ always visible like ProductFilter; disabled if nothing selected -->
          <button type="button" class="btn-clear" :disabled="!hasAny" @click="clearAll">
            Clear
          </button>

          <!-- Mobile main collapse toggle -->
          <button
            v-if="isMobile"
            type="button"
            class="btn-toggle"
        ...
        <div class="filters-body-inner">
          ...
          <div class="filter-block">
            ...
            <div id="sortPanel" class="filter-content" :class="{ 'is-open': sortOpen }">
              <div class="filter-content-inner">
                <label class="opt">
                  <input type="radio" name="sort" value="latest" v-model="sortLocal" @change="onSortPick" />
                  <span class="radio-custom"></span>
                  <span class="opt-label">Latest</span>
                </label>

                <label class="opt">
                  <input type="radio" name="sort" value="price_asc" v-model="sortLocal" @change="onSortPick" />
                  <span class="radio-custom"></span>
                  <span class="opt-label">Price: Low to High</span>
                </label>

                <label class="opt">
                  <input type="radio" name="sort" value="price_desc" v-model="sortLocal" @change="onSortPick" />
                  <span class="radio-custom"></span>
...
<script>
export default {
  name: "ProductFilter", // keep your name
  ...
  data() {
    return {
      ...
      // prevent double apply when syncing from props
  ...
  mounted() {
    if (typeof window !== "undefined" && window.matchMedia) {
      ...
      const applyMode = (matches, force = false) => {
        ...
        // ✅ same as ProductFilter: closed on mobile, open on desktop
  ...
  methods: {
    ...
    apply() {
      ...
      this.$inertia.get(route("specialoffers.index"), params, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
      });

      // ✅ same as ProductFilter: do NOT auto-close on mobile
    },
...
/* ========== MAIN CARD (ProductFilter design) ========== */
.filters {
...
.filters * { font-family: inherit; }

.filters:hover { box-shadow: var(--filter-shadow-hover); }

.filters.is-desktop-sticky { position: sticky; top: 16px; }
...
.filter-block:first-child { border-top: none; padding-top: 8px; }
...
.filter-head:hover { background: var(--filter-hover); }
...
.filter-content.is-open { grid-template-rows: 1fr; opacity: 1; }
.filter-content-inner { overflow: hidden; padding-top: 8px; }
...
.opt:hover { background: var(--filter-hover); }
...
.opt input[type="radio"]:checked + .radio-custom { border-color: var(--filter-accent); }
.opt input[type="radio"]:checked + .radio-custom::after { background: var(--filter-accent); transform: scale(1); }
...
.opt input[type="checkbox"]:checked + .checkbox-custom { border-color: var(--filter-accent); background: var(--filter-accent); }
.opt input[type="checkbox"]:checked + .checkbox-custom::after { opacity: 1; transform: scale(1); }

/* ========== RESPONSIVE ========== */
@media (max-width: 768px) {
  .filters { padding: 14px; }
  .filter-block { padding: 10px 0; }
  .filter-value { max-width: 100px; }
  .opt { padding: 10px 8px; margin: 2px -8px; }
}
@media (max-width: 400px) {
  .filters { padding: 12px; border-radius: 12px; }
  .filters-header { padding-bottom: 10px; }
  .filters-title { font-size: 0.9375rem; }
  .filter-value { max-width: 80px; font-size: 0.75rem; }
}
</style>
